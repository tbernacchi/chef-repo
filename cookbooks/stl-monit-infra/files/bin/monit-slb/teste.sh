...
# LOCKFILE
LOCK="/tmp/slb-vip.lck-teste"

# TEMP File
TEMP_FILE="/tmp/db-vip.temp-teste"

# GROUP_FILES
TEMP_SG="/tmp/db-vip.sg-teste"

# File Status
TEMP_STATUS="/tmp/db-vip.status-teste"
...
fn_create_vip_zabbix()
  {
   for vip_host in `cat $TEMP_FILE |  grep -w "name" | awk '{ print $2 }' | sed -e 's/"//g' -e 's/,//g'`
    do
      IP=`cat $TEMP_FILE | grep -w $vip_host | awk '{ print $2 }' | sed -e 's/"//g' -e 's/,//g'`

      echo "nome do VIP $vip_host e o IP $IP"
    done 
  }
