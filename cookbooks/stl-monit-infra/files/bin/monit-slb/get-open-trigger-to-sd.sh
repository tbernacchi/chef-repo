...
fn_sanatizar()
	...
	for trigger in `ls $WORKDIR | grep $TRG_FILE`
		do
			FILE_DESCRIPTION=`cat $WORKDIR/$trigger | grep jsonrpc | jq -r '.' | grep -w expression | awk '{ print $2 }' | sed 's/"//g' | cut -f2 -d"{" | sed -e 's/://g' -e 's/\[//g' -e 's/\]//g' -e 's/\///g' -e 's/,//g' -e 's/(//g' -e 's/)//g' -e 's/{//g' -e 's/}//g' -e 's/>//g' -e 's/<//g' -e 's/\=//g' -e 's/\#//g' -e 's/\%//g'`
